...
export default function AdminStudentListPage() {
  ...
  useEffect(() => {
    let courseNameFromHome = '';
    let sectionFromHome = '';
  
    if (searchParams.has('courseName')) {
      const courseNameParam = searchParams.get('courseName');
      const params = courseNameParam.split('section=');
      if (params.length === 2) {
        courseNameFromHome = params[0];
        sectionFromHome = parseInt(params[1]);
        console.log('courseNameFromHome : ' + courseNameFromHome);
        console.log('sectionFromHome : ' + sectionFromHome);
      }
    
    }
  
    setCourseName(courseNameFromHome);
    setSection(sectionFromHome);
  }, [searchParams]);
...
//      const default_hash = await gradeTokenOffering('default', 0, 'default', 'notUse', 7, 7, 'notUse');
//      console.log("default: :", default_hash);
      //console.log("ipfs_hash in studentlist file: " + ipfs_hash);
      closeGrade();;
        //console.log('defaultAccount: ' + defaultAccount);
     // safeMinting('https://ipfs.io/ipfs/QmPpHMtYikc1zix5F9diGd4dqpkgLNbhTQfEr4SceFCgCW', stdAddress);
