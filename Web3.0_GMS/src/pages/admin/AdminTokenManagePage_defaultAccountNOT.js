...
export default function HomePage() {
  ...
  const goStudentList = (courseName) => {
    
    //history.pushState("/a_dashboard/a_studentlist",{courseName});
  ...
  .then((response) => {
    defaultAccount = response.data[0].walletAddress;
  ...
  const submitToken = async () => {
	...
	console.log('inside submittoken');
	console.log(defaultAccount);
	safeMinting(ipfs_hash, defaultAccount);
//        const tokenidFromSafemint = await safeMinting(ipfshash, defaultAccount);
//        console.log('token id : ' + tokenidFromSafemint);
        // 여기서 tokenidFromSafemint 값을 사용할 수 있습니다.
    } catch (error) {
        console.log(error);
    }
};
