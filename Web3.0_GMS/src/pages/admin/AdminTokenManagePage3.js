...
//import { safeMinting } from '/home/ubuntu/Test/Web3.0_GMS/src/backend/safeMint';
...
export default function HomePage() {
  ...
  useEffect(() => {
    axios.get(`/a_dashboard/a_tokenmanage/professor/`)
  .then((response) => {
    const account = response.data[0].walletAddress;
  ...
  const submitToken = () => {
    ...
    .then(async () => {
      ...
      //data.js에서 ipfs 가져오기
//      const ipfs_hash = await tokenOffering(courseName, section, type, tokenName, totalScore);
//      const tokenIdFromSafeMint = safeMinting(ipfs_hash, defaultAccount);
//      console.log('tokenId:', tokenIdFromSafeMint);
//      console.log('tokenId: ', await safeMinting(ipfs_hash, defaultAccount));
  ...
  const handleClickOpenForEdit = async (courseName, tokenName, type, section, totalScore, tokenID) => {
    setForEdit(true);
                  ...
                  {list.map((row) => (
                    <TableRow key={row.tokenID}>
                      ...
                      <TableCell align='center'>
                        <Button className='revision' onClick={() => handleClickOpenForEdit(row.courseName, row.tokenName, row.type, row.Section, row.totalScore, row.tokenID)}>수정</Button>
